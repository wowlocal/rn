...
def main() -> int:
    parser = argparse.ArgumentParser(description="Summarize Discord IPA React Native version ranges.")
    parser.add_argument("--report", type=Path, default=Path("discord-rn-versions.csv"))
    parser.add_argument("--out-prefix", type=Path, default=Path("discord-rn-timeline"))
    ...
    args.out_prefix.with_suffix(".json").write_text(json.dumps(data, indent=2) + "\n")
