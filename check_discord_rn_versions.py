...
def list_versions(args: argparse.Namespace) -> list[VersionEntry]:
    if args.versions_json:
        ...
        (args.download_dir / "discord-version-list.json").write_text(
...
def download_ipa(args: argparse.Namespace, entry: VersionEntry, index: int) -> Path:
    ...
    out = args.download_dir / f"discord_{index:03d}_{label}_{entry.external_version_id}.ipa"
...
def analyze_ipa(path: Path) -> dict[str, str]:
    ...
    notes: list[str] = []
    with tempfile.TemporaryDirectory(prefix="discord-rn-") as tmp_name:
        tmp = Path(tmp_name)
        with zipfile.ZipFile(path) as zf:
...
def collect_ipas(args: argparse.Namespace, downloaded: list[Path]) -> list[Path]:
    ...
    if args.analyze_existing:
        ...
        if Path("Discord.ipa").exists():
            paths.append(Path("Discord.ipa"))
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Download Discord iOS app versions with ipatool and infer React Native versions from the IPAs."
    )
    ...
    parser.add_argument("--download-dir", type=Path, default=Path("discord-ipas"))
    parser.add_argument("--report", type=Path, default=Path("discord-rn-versions"))
    ...
    parser.add_argument(
        "--skip-list-versions",
        action="store_true",
        help="Do not call ipatool list-versions; read external IDs from --seed-ipa or ./Discord.ipa.",
    )
    ...
    parser.add_argument("--print-version-ids", action="store_true", help="Print external version IDs from --versions-json, --seed-ipa, or ./Discord.ipa and exit.")
    ...
    parser.add_argument("--analyze-existing", action="store_true", default=True, help="Analyze IPAs in the download dir and ./Discord.ipa.")
...
def main() -> int:
    ...
    if args.print_version_ids:
        if args.version_id:
            ...
            seed_ipas = args.seed_ipa or ([Path("Discord.ipa")] if Path("Discord.ipa").exists() else [])
