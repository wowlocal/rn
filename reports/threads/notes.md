...
- `373.0.0` build `711845629`, external ID `873427556`: source IPA SHA-256 `1f7d1f73d97ce24d5566abd9cf73198782a6e32f7e99c147b53550876f36028a`, dumped IPA SHA-256 `b216418958cddd8b6e3b396799f034982f346fd70b8dd075b7db8a30b6404518`, dumped size `49844742`, metadata matched source bundle/version/build, main executable `cryptid 0`, coverage `loaded_app_decrypted`.
- `387.0.0` build `755202338`, external ID `875824747`: source IPA SHA-256 `60f63a1f0e1f3418e932d9e75c22d80bbae2ab1ec966eac4d0097024697f981b`, dumped IPA SHA-256 `3c998e30b8d795fa19c019e9bd05f6ff744e93e525b8287da19a4ff65a551888`, dumped size `58324416`, metadata matched source bundle/version/build, main executable `cryptid 0`, coverage `loaded_app_decrypted`.
...
- Per-Mach-O inventory for `373.0.0`: 4 Mach-Os total; main executable decrypted; 3 app-extension executables remain encrypted.
- Per-Mach-O inventory for `387.0.0`: 4 Mach-Os total; main executable decrypted; 3 app-extension executables remain encrypted.
...
- Remaining encrypted executables are `BarcelonaNotificationExtension`, `BarcelonaShareExtension`, `BarcelonaWidgetExtension`, and `BarcelonaWidgetExtensionLiveActivities`. Extension attach was not pursued because the main executable already contains strong RN-related native evidence and no current evidence suggests the RN version marker only lives in extensions.
- Decrypted analyzer result for all four accepted rows: no JS bundle, Hermes bytecode, `react-native-renderer`, `BatchedBridge`, `AppRegistry`, version-specific RN marker, strong Hermes marker, or strong JSI marker was found; the decrypted main executable does contain strong RCT class markers such as `RCTModalHostViewController`/`RCTJavaScriptContext` plus Yoga markers. RN presence is therefore stronger for these four iOS rows, but exact RN version remains `unknown` with reason `native_rn_marker_without_version`.
- The 8 other sampled iOS rows remain unresolved with reason `encrypted_native_only`; each row now carries a next action to dump with `frida-ipa-extract --all-binaries` only if it becomes boundary-priority evidence.
...
- iOS decrypted evidence for app `373.0.0`, `387.0.0`, and `400.0.0` still contains native RN references, so the Android marker disappearance should be treated as analyzer/source-limited rather than proof that Threads removed RN by that release train.
...
- Exact RN patch versions are not recoverable from the Android native metadata, encrypted iOS samples, or the four accepted decrypted iOS dumps currently available.
...
Manual review is still needed to access trusted APKMirror or another signature-verified source for Android builds between `382.0.0.51.85` and `400.0.0.38.68`. Further iOS work should look for stronger native RN version constants or a triggerable RN surface that loads version-specific code before dumping broad history.
