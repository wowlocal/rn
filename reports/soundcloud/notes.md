...
- `7.16.0` build `1243764`, external ID `860769428`: source IPA SHA-256 `974817fa13e2adfe9a6f8a1963421abb5be40ad3ab9825810e2b520ba6fd5208`, dumped IPA SHA-256 `d62465b12bfa4f72c43b8cb64d97a822deadd7be5d0c582405f256ded36d7e6c`, dumped size `81143948`, metadata matched source bundle/version/build, main executable `cryptid 0`, coverage `loaded_app_decrypted`.
...
- `7.16.0` Mach-O inventory: 61 Mach-Os total; main executable and 58 bundled framework/dylib binaries decrypted; 2 widget app-extension executables remain encrypted; no non-extension Mach-Os remain encrypted.
- Reanalysis with stricter native-marker rules found the earlier native React Native/JSI result was a false positive from broad substring matching. Weak strings include `ios_react_native`/`ios_reactnative`, isolated `RCTView`, and incidental `JSi` text; there are no strong `RCTBridge`/`RCTRootView`/`facebook::react`/`ReactCommon`/`React-Core` markers.
- Decrypted analysis for `7.16.0`, `7.44.0`, `8.5.0`, `8.32.0`, and `8.62.0` finds no RN JS bundle, Hermes bytecode, `react-native-renderer`, version-specific RN marker, strong native RN symbol, or strong native Hermes/JSI/Yoga marker. The rows remain RN `unknown` with reason `no_js_or_native_rn_markers`.
- Remaining encrypted non-extension code in later dumps is limited to Watch app binaries such as `Payload/SoundCloud.app/Watch/SCWatch.app/Frameworks/SwiftSoundCloud.framework/SwiftSoundCloud`, `Payload/SoundCloud.app/Watch/SCWatch.app/Frameworks/Atomics.framework/Atomics` in `8.32.0`, and `Payload/SoundCloud.app/Watch/SCWatch.app/SCWatch`. App extensions also remain encrypted, but extension triggering was not pursued because no sampled source row or decrypted main-app/framework evidence points to RN living there.

## Result

Marked `not_react_native_detected` because no React Native markers were found in the 12-point iOS source sample, five accepted decrypted iOS main-app samples from `7.16.0` through `8.62.0`, or the duplicated current Android payloads. Earlier source-only iOS rows remain encrypted, but the current dataset no longer contains reliable React Native evidence for SoundCloud.
